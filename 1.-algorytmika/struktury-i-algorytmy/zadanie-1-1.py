...
def szesnastkowa_do_dziesiatna():
    ...
    print(f'szesnastkowa: {szesnastkowa} => dzisietna: {suma}', end='\n\n')


def dzisietna_do_szesnastkowa():
    ...
    dziesietna = '255'
    dziesietna_l = int(dziesietna)
    cyfry_16 = []
    while dziesietna_l != 0:
        (dziesietna_l, mod) = divmod(dziesietna_l, 16)
        wartosc = slownik_10_do_16[mod]
        cyfry_16.append(wartosc)
    cyfry_16.reverse()
    szesnastkowa = "".join(cyfry_16)
    print(
        f'dziesietna: {dziesietna} => szesnastkowa: {szesnastkowa}', end='\n\n')
