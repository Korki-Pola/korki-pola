...
def main():
    ...
    # potrzebne tylko do celow sprawdzenia poprawnego dzialania programu
    # print(random_word)
    ...
    while not player_won and not player_lost:
        ...
        if is_letter_correct:
            ...
            print('Twoja litera nie jest poprawna')
            number_of_tries -= 1
...
def fill_guess(letter: str, random_word_chars: list[str], floors_list: list[str]) -> list[str]:
    ...
    while some_letter_still_left:
        try:
            # podniesie ValueError jezeli nie znajdzie letter
...
def check_if_letter_correct(letter: str, random_word_chars: list[str]) -> bool:
    try:
        # podniesie ValueError jezeli nie znajdzie letter
        random_word_chars.index(letter)
        return True
    except ValueError:
        return False
...
def get_letter_from_user() -> str:
    ...
    while is_input_incorrect:
        ...
        # sprawdzenie czy uzytkownik podal pojedynczy znak (oraz wyrzucenie spacji)
        if len(letter) != 1:
